...
async def process_event(
    ...
    server_log.debug(f'Process Event Callback id:{message_in.id} status:{message_in.status_id}')
    bets_with_event: list[Bet] = \
        await crud_bet.get_by_event_id(
            db, event_id=message_in.id
        )
    for bet in bets_with_event:
        ...
        await crud_bet.update(
            db,
            db_obj=bet,
            obj_in=crud_schemas.BetUpdate(id=bet.id, status_id=bet_status)
        )
