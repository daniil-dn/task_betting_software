...
class TestEvent:
    ...
    def test_create_bet(self) -> None:
        r = requests.get(f"http://app_bot_maker:9090{settings.API_V1_STR}/events")
        ...
        r = requests.post(f"http://app_bot_maker:9090{settings.API_V1_STR}/bet", json=bet_data)
        assert r.status_code == 200

    def test_get_bets(self) -> None:
        r = requests.get(f"http://app_bot_maker:9090{settings.API_V1_STR}/bets")
        assert r.status_code == 200
